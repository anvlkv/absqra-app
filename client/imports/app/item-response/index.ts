import {ItemResponseComponent} from "./item-response.component";
import {ItemResponseFormInputComponent} from "./item-response-form-input.component";
import {ItemResponseFormTextareaComponent} from "./item-response-form-textarea.component";
import {ItemResponseFormRadioComponent} from "./item-response-form-radio.component";
import {ItemResponseFormCheckboxComponent} from "./item-response-form-checkbox.component";
import {ItemResponseFormListingComponent} from "./item-response-form-listing.component";
import {ItemResponseFormOrderingComponent} from "./item-response-form-ordering.component";
import {ItemResponseFormBtnGroupComponent} from "./item-response-form-btn-group.component";
/**
 * Created by a.nvlkv on 19/11/2016.
 */

export const ITEM_DECLARATIONS = [
    ItemResponseComponent,
    ItemResponseFormInputComponent,
    ItemResponseFormTextareaComponent,
    ItemResponseFormRadioComponent,
    ItemResponseFormCheckboxComponent,
    ItemResponseFormListingComponent,
    ItemResponseFormOrderingComponent,
    ItemResponseFormBtnGroupComponent
]